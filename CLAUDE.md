...
| [docs/prd.md](docs/prd.md) | **Source of truth**: stories, decisions P1–P16, scope, build order |
...
Work = GitHub issues, dependency-ordered. Don't start an issue whose blockers aren't closed. The v1 build order (**#12–#23**) and the pre-launch hardening track (**#57–#95**) are closed; what remains open is **HITL only** (#11, #56, #59, #63, #64, #65, #66, #67) — the founder provisions infra, domain, e-mail and provider keys.
...
- **Current state (2026-08-07) — pre-launch hardening track, all merged.** The v1 build order (#12–#23) closed in July; everything since is hardening for launch, and every layer doc was updated in the same PR as the code. What shipped, and where it is written down:
  - **Trust surface & auth (#57, #58, #68, #69, #61, #59-adjacent):** public `/privacidade` + `/termos` (copy as structured data in `app/(legal)/copy.ts`, one `LegalDocument` presenter, sub-processors as a stacked card list — docs/frontend.md §9.6); one password rule for the whole product in `lib/auth/password.ts` (length + breach, **no** composition rule — `PASSWORD_MIN = 10`, `PASSWORD_MAX_BYTES = 72`); forgot-password (`/auth/recuperar` → `/auth/nova-senha`) whose request step is an enumeration dead end (one constant reply + a 900ms floor) and whose reset is gated by a 15-minute httpOnly **recovery grant** stamped by `/auth/callback`, plus `safeNextPath()` against open redirects; change-password on `/configuracoes` verifying the current password on a throwaway non-persisting client and evicting other sessions; Postgres-backed rate limiting on the invitation create/accept paths (`rate_limit_take` under an advisory lock, hashed subjects, **fails open**). All in docs/backend.md §8.
  - **Evidence & data rights (#73, #74):** `audit_log` (append-only, Admin-read, actor snapshotted by e-mail, redaction chokepoint, non-fatal writes, 24-month retention) at `/ajustes/auditoria`; LGPD art. 18 self-service at `/ajustes/privacidade` — a streamed allowlisted JSON export honouring both privacy switches, and `deleteTenantPermanently()` in a deliberate order (revoke connections → delete Auth users → delete tenant, cascade). docs/backend.md §8, docs/prd.md → Data & security.
  - **Closed months (#94, #95):** `period_snapshot` freezes each finished month (seat state is current state and cannot be recomputed later), built by the pure `buildPeriodSnapshot()` reusing the existing engine; closing runs in the daily cron after sync + alerts, idempotent, backfilling at most one month per tenant per run. `/relatorios` + `/relatorios/[period]` read **only** the frozen row, and the shell skips its live cockpit reads on those paths. docs/backend.md §15, docs/frontend.md §9.7.
  - **Platform (#60, #75, #76, #78, #79, #80, #70, #71, #72, #77, #86, #87):** security headers + CSP (docs/architecture.md §4b); rotatable credential keyring with key id + AAD and a rehearsed rotation procedure (docs/backend.md §14); supply-chain gate — one reusable `quality.yml` (install → audit → lint → typecheck → **database** → test → **build**) consumed by both CI and deploy, SHA-pinned actions, Dependabot, and the lockfile generated on Linux (docs/architecture.md §10); the RLS isolation suite now runs in CI against an ephemeral Supabase stack and a **skip counts as a failure** (`DENARIUS_REQUIRE_DB_TESTS=1`); structured one-line JSON server logs with a redaction chokepoint and a short user-quotable `Referência` (docs/backend.md §16); the responsive contract at 375/768 in both themes (docs/frontend.md §5); error boundaries, branded not-found, drill-down skeleton, role-gated settings pages, and the internal button reference kept out of production.
  - **Data API privileges are explicit** (`20260807110000_api_privileges.sql`): grants are declared per table/function in migrations instead of inherited from an old Supabase project default. Every new table or function migration must grant its intended role **in the same file** — RLS and SQL privileges are two independent controls (docs/architecture.md §4).
  - **Migrations:** the chain now runs through `20260807130000_data_rights.sql`. `20260807120000_period_snapshot.sql` was applied to the hosted project on 2026-08-07; before relying on `/ajustes/auditoria`, `/ajustes/privacidade/exportar` or `/relatorios` in prod, confirm `20260805170000_audit_log.sql`, `20260807110000_api_privileges.sql` and `20260807130000_data_rights.sql` are applied there too — the audit screen deliberately shows a "could not load" state rather than claiming an empty trail.
  - **Open work is HITL only** (issues #11, #56, #59, #63, #64, #65, #66, #67): real provider Admin keys, the production domain + DNS, Supabase Auth hardening, **splitting the production Supabase project from development** (prod and local dev still share one — docs/architecture.md §8 and this file still say so, and #63 asks for both to be updated when it lands), the Resend sending domain, the Google consent screen, pt-BR auth e-mail templates, and the go-live checklist (#67, which explicitly requires this current-state section to be updated to the launched configuration).
