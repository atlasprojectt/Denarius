...
- **Current state (2026-07):** docs + prototype done; PRD hardened (P1–P15, F1–F6 locked). **Merged: #12 walking skeleton** (auth + tenant + RLS, isolation test 5/5) and **#13 roster CSV** (pure parser, atomic `roster_import` RPC, preview UI, inline edit — 23/23 tests). Both migrations applied on Supabase (manually via SQL editor — check whether the GitHub integration is also active before assuming auto-apply). Next: #14 (manual seats + attribution dashboard). Admin keys for the provider spike (#11) still pending (needs org Owner). `VERCEL_TOKEN` available in `.env.local` for CLI deploys; Vercel project not linked yet. CodeRabbit reviews every PR.
