...
- **Current state (2026-07):** docs + prototype done; PRD hardened (P1–P15, F1–F6 locked). **Merged: #12 walking skeleton** (auth + tenant + RLS, isolation test 5/5), **#13 roster CSV** (pure parser, atomic `roster_import` RPC, preview UI, inline edit), **#14 manual seats** (subscription CRUD, daily-accrual engine in `lib/engine/accrual.ts`, Explore by-team + Unattributed + reconciliation footer). Migrations #12/#13 applied on Supabase manually via SQL editor; **#14's `*_subscriptions.sql` needs the same manual apply** (check whether the GitHub integration is active before assuming auto-apply). Next: #15 (OpenAI connector). Admin keys for the provider spike (#11) still pending (needs org Owner). `VERCEL_TOKEN` available in `.env.local` for CLI deploys; Vercel project not linked yet. CodeRabbit reviews every PR.
