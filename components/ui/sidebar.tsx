...
import { RiSidebarFoldLine } from "@/components/domain/icons"
...
function SidebarInput({
  ...
  return (
    <Input
      ...
      data-sidebar="input"
      className={cn(
        "h-8 w-full border-input bg-input/20 dark:bg-input/30",
        className
      )}
      {...props}
    />
...
function SidebarGroupAction({
  ...
  return useRender({
    ...
    props: mergeProps<"button">(
      {
        className: cn(
          "absolute top-3.5 right-3 flex aspect-square w-5 items-center justify-center rounded-full p-0 text-sidebar-foreground ring-sidebar-ring outline-hidden transition-transform group-data-[collapsible=icon]:hidden after:absolute after:-inset-2 hover:bg-surface-hover hover:text-sidebar-accent-foreground focus-visible:ring-2 md:after:hidden [&>svg]:size-4 [&>svg]:shrink-0",
          className
        ),
      },
      props
...
function SidebarMenuButton({
  ...
  const comp = useRender({
    ...
    render: !tooltip ? childRender : <TooltipTrigger render={childRender} />,
    state: {
