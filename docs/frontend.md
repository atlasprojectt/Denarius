...
7. **Dashboard panels**: "Para onde vai o dinheiro" (provider/seat composition as **ranked horizontal tick bars** with provider brand marks — the donut was retired 2026-07-14; neutral chart-ramp colors, share-cut fills, beside the hero) and the monthly pace line (bottom-left grid cell, current spend + linear projection). Team distribution stays in the teams table, not as a second composition panel. The 2x2 grid uses **content-sized rows and compact 16px gaps**: cards match the height of their row without the tallest card forcing both rows to grow. The analytical second row is deliberately taller than the summary row: the pace plot is bounded to 220–240px and the half-width team cards use compact rows, keeping the normal desktop cockpit inside the target 1440×900 viewport without page scroll while preserving every value.
...
| `NextActionsButton` | Global-header "Próximas ações" popover: tonal brand-accent pill with a count badge (one prefetch per page load; items refetched on open) |
