...
- **Historical/multi-period forecasting** and **anomaly detection** (need accumulated history). *Intra-period **linear run-rate projection** is IN — it needs no history.* **Also IN (#94/#95): DISPLAYING a closed month.** Freezing each month into `period_snapshot` and printing it as a report is documentation of what happened, not a model of what will happen — nothing in the product reads more than one period to compute a number, and the run-rate projection stays intra-period. The line this scope item draws is between *showing* history and *forecasting on* it; only the second is out. **Also IN (#96): the SAME report for the month still running**, generated on demand at `/relatorios/agora` — it reads one period, projects the close at intra-period run-rate under the day-5 guard, and is never persisted. A report that spanned periods, or that forecast beyond the current month, would cross the line; this one does not. **#97** then made both reports executive documents: a deterministic composer (`lib/reports/compose.ts`) writes the summary, ranks by materiality and decides emphasis over a fixed §1–§6 spine, and the live report carries recommended actions from the curated catalog while the closed one states the same crossings as observations. Still one period, still no LLM (invariant #2) — the document says so in its own annex.
