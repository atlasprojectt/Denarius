...
- Left sidebar, **6 destinations: Home / Search / Times / Explore / Reports / Settings.** Search is a server-side workspace navigator over real, authorized resources; it reduces navigation without becoming a command palette. Budgets and Planning remain excluded — they duplicate Home or break the causal chain (see below).
...
- **Historical/multi-period forecasting** and **anomaly detection** (need accumulated history). *Intra-period **linear run-rate projection** is IN — it needs no history.* **Also IN (#134, founder-approved 2026-09-04): the intra-period behavior-aware close (Forecast Engine v2).** It still reads only the current period plus the immediately trailing days as transient context (recent-pace weighting, outlier capping, weekday recurrence with a minimum sample) — no persisted model, no cross-period training, no read of more than one closed month to compute a number. The day-5 guard, USD source of truth, and uncosted disclosure all still apply. The line this scope item draws is between *closing the current month smarter* (in) and *modeling future months from history* (out). **Also IN (#94/#95): DISPLAYING a closed month.** Freezing each month into `period_snapshot` and printing it as a report is documentation of what happened, not a model of what will happen — nothing in the product reads more than one period to compute a number, and the run-rate projection stays intra-period. The line this scope item draws is between *showing* history and *forecasting on* it; only the second is out. **Also IN (#96): the SAME report for the month still running**, generated on demand from `/relatorios` (preview dialog; the former route survives only as the headless PDF render endpoint) — it reads one period, projects the close at intra-period run-rate under the day-5 guard, and is never persisted. A report that spanned periods, or that forecast beyond the current month, would cross the line; this one does not.
